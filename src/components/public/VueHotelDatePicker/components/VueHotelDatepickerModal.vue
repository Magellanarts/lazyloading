<template>
  <div class="date-range-picker-modal" @click="cancel">
...
export default {
  ...
  methods: {
    ...
    generateCalendar(calculateYear = new Date().getFullYear(), calculateMonth = new Date().getMonth(), config = {}) {
      ...
      const countTime = new Date(calculateYear, calculateMonth, 1, 0, 0, 0);
      const tempMonthAry = [];
      let tempWeekAry = [];
      let nextMonth = false;
      let completed = false;
      while (!nextMonth || (nextMonth && !completed)) {
        const day = countTime.getDay();
        const date = countTime.getDate();
        const month = countTime.getMonth();
        // check next month
